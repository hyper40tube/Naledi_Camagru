...
$stmt = $con->prepare("SELECT * FROM `likes` INNER JOIN images ON images.image_id = likes.image_id WHERE images.image = :image");
			...
			<div class = "row">
				<?php
					...
					while ($likes = $stmt->fetch(PDO::FETCH_ASSOC)) {
						$user = $likes["username"];
						echo "<h7 style = 'font-size: 15px;'> $user </h7>
							 <br/>
							 <br/>";
					}
					echo "</div>
